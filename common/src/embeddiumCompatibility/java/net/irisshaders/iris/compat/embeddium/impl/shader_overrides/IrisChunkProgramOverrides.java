...
import net.irisshaders.iris.pipeline.EmbeddiumTerrainPipeline;
...
public class IrisChunkProgramOverrides {
	...
	private List<BufferBlendOverride> getBufferBlendOverride(IrisTerrainPass pass, EmbeddiumTerrainPipeline pipeline) {
		if (pass == IrisTerrainPass.SHADOW || pass == IrisTerrainPass.SHADOW_CUTOUT) {
	...
	@Nullable
	private GlProgram<IrisChunkShaderInterface> createShader(IrisTerrainPass pass, EmbeddiumTerrainPipeline pipeline, ChunkVertexType vertexType) {
	...
	private float getAlphaReference(IrisTerrainPass pass, EmbeddiumTerrainPipeline pipeline) {
		if (pass == IrisTerrainPass.SHADOW || pass == IrisTerrainPass.SHADOW_CUTOUT) {
	...
	private EmbeddiumTerrainPipeline getEmbeddiumTerrainPipeline() {
		WorldRenderingPipeline worldRenderingPipeline = Iris.getPipelineManager().getPipelineNullable();

		if (worldRenderingPipeline != null) {
			return worldRenderingPipeline.getEmbeddiumTerrainPipeline();
		} else {
			return null;
		}
	}

	public void createShaders(EmbeddiumTerrainPipeline pipeline, ChunkVertexType vertexType) {
		if (pipeline != null) {
	...
	public GlProgram<IrisChunkShaderInterface> getProgramOverride(TerrainRenderPass pass, ChunkVertexType vertexType) {
		...
		WorldRenderingPipeline worldRenderingPipeline = Iris.getPipelineManager().getPipelineNullable();
		EmbeddiumTerrainPipeline EmbeddiumTerrainPipeline = null;

		if (worldRenderingPipeline != null) {
			EmbeddiumTerrainPipeline = worldRenderingPipeline.getEmbeddiumTerrainPipeline();
		}

		if (!shadersCreated) {
			createShaders(EmbeddiumTerrainPipeline, vertexType);
		}

		if (ShadowRenderingState.areShadowsCurrentlyBeingRendered()) {
			if (EmbeddiumTerrainPipeline != null && !EmbeddiumTerrainPipeline.hasShadowPass()) {
	...
	public void bindFramebuffer(TerrainRenderPass pass) {
		EmbeddiumTerrainPipeline pipeline = getEmbeddiumTerrainPipeline();
